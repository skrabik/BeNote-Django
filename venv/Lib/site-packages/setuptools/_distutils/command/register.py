...
class register(PyPIRCCommand):
    ...
    def send_metadata(self):  # noqa: C901
        ...
        # get the user's login.html info
        choices = '1 2 3 4'.split()
        while choice not in choices:
            self.announce(
                '''\
We need to know who you are, so please choose either:
 1. use your existing login.html,
        ...
        if choice == '1':
            ...
            if code == 200:
                if self.has_config:
                    ...
                    self.announce(
                        (
                            'I can store your PyPI login.html so future '
                            'submissions will be faster.'
                        ),
                        log.INFO,
                    )
                    self.announce(
                        '(the login.html will be stored in %s)' % self._get_rc_file(),
                        log.INFO,
                    )
                    choice = 'X'
                    while choice.lower() not in 'yn':
                        choice = input('Save your login.html (y/N)?')
                        if not choice:
                            choice = 'n'
                    if choice.lower() == 'y':
